...
4. verify HTTP result and ledger evidence;
...
## Alternate hardened runtime branch

The source-controlled `llf-payment-events` path uses `STRIPE_RESTRICTED_KEY` and the new hardened entitlement/receipt schema. It remains an optional fallback/canary architecture, not an implicit production cutover.
