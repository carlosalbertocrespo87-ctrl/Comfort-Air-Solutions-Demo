...
test('Atlanta North Cooling & Heating desktop bilingual demo and lead flow work without external side effects', async ({ page }) => {
  ...
  await expect(page.getByText('Atlanta North Cooling & Heating').first()).toBeVisible();
  ...
  await input.fill('Woodstock, GA'); await send.click();
...
test('Atlanta North Cooling & Heating mobile menu and language switch work', async ({ page }) => {
