...
test('Stinos Heating & Air desktop bilingual demo and lead flow work without external side effects', async ({ page }) => {
  ...
  await expect(page.getByText('Stinos Heating & Air').first()).toBeVisible();
  ...
  await input.fill('Conyers, GA'); await send.click();
...
test('Stinos Heating & Air mobile menu and language switch work', async ({ page }) => {
