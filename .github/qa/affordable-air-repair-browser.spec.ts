...
test('Affordable Air Repair desktop bilingual demo and lead flow work without external side effects', async ({ page }) => {
  ...
  await expect(page.getByText('Affordable Air Repair').first()).toBeVisible();
  ...
  await input.fill('Smyrna, GA'); await send.click();
...
test('Affordable Air Repair mobile menu and language switch work', async ({ page }) => {
