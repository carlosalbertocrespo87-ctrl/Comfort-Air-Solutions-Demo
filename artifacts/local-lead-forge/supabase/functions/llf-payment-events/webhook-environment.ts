...
export type WebhookSignatureModeResult = { ok: true; mode: VerifiedWebhookMode } | { ok: false; error: 'stripe_signature_invalid' };
...
export async function verifyWebhookSignatureMode(input: {
  ...
  const live = await verifyStripeSignature(input.rawBody, input.signatureHeader, input.liveSecret, 300, nowSeconds);
  if (live.ok) return { ok: true, mode: 'live' };
  if (!input.testSecret) return { ok: false, error: 'stripe_signature_invalid' };
  const test = await verifyStripeSignature(input.rawBody, input.signatureHeader, input.testSecret, 300, nowSeconds);
  return test.ok ? { ok: true, mode: 'test' } : { ok: false, error: 'stripe_signature_invalid' };
}
