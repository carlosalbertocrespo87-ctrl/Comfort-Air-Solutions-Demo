...
export function AgentMacroDrawer({ language, context, latestCustomerMessage = '', disabled = false, onInsert }: AgentMacroDrawerProps) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [favorites, setFavorites] = useState<string[]>(['pricing-overview', 'support-availability']);
  ...
  const visible = useMemo(() => {
    ...
    return [...matches].sort((a, b) => Number(favorites.includes(b.id)) - Number(favorites.includes(a.id)));
  }, [query, language, favorites]);
