import { Archive, BookOpenCheck, PlusCircle } from 'lucide-react';
...
export function ControlledLearningPanel({ items, candidateQuestion = '', disabled = false, language, onQueueCandidate, onArchive }: ControlledLearningPanelProps) {
  ...
  return (
    <section className="mt-3 rounded-xl border border-violet-500/20 bg-violet-500/[0.04] p-3">
      ...
      <p className="mt-2 text-[8px] leading-4 text-slate-500">Approved content is never created automatically. Review and explicit approval remain required.</p>
    </section>
  );
}
