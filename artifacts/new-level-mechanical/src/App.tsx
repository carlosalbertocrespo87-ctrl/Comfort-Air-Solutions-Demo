...
const prospectConfig = {
  ...
  shortName: "NEW LEVEL MECHANICAL",
  logoTagline: "",
  emailDomain: "newlevelmechanical.com",
  phoneDisplay: "(404) 723-3830",
  serviceArea: "Marietta & North Atlanta",
  sinceYear: "",
  businessHours: "",
  serviceCities: [
    "Marietta",
  ...
  serviceAreaItems: [
    ...
    "Alpharetta, Kennesaw & Acworth",
    "Woodstock & North Atlanta",
  ],
} as const;
...
const translations = {
  en: {
    ...
    area: {
      ...
      cities: prospectConfig.serviceCities,
    },
    contact: {
  ...
  es: {
    ...
    hero: {
      eyebrow: `El equipo local de confort de ${prospectConfig.serviceArea}`,
      title1: "Servicio de HVAC",
      title2: "rápido y confiable",
      title3: `en ${prospectConfig.serviceArea}`,
      subhead:
      ...
      items: [
        {
          ...
          copy: `Servicio HVAC confiable en ${prospectConfig.serviceArea}. Encontramos la causa del problema y te la explicamos claramente.`,
          detail:
    ...
    area: {
      ...
      copy: `Nos enorgullece servir comunidades en ${prospectConfig.serviceArea}. Si estás cerca y no estás en la lista, llámanos — con gusto conversamos.`,
      phone: prospectConfig.phoneDisplay,
...
function Area({ lang }: { lang: Language }) {
  ...
  const cities = prospectConfig.serviceCities;
  return (
    <section
      id="area"
    ...
    <div className="noise min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      >
        <input
          ...
          className={`${demoExpanded ? "block" : "hidden"} min-w-0 flex-1 rounded-full bg-[hsl(var(--background))] px-3 py-2 text-sm text-[hsl(var(--foreground))] shadow-sm sm:block sm:flex-none`}
        />
        <button
          onClick={() => {
