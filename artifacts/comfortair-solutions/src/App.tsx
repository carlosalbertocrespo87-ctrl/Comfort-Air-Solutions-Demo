...
const translations = {
  en: {
    ...
    why: {
      ...
      features: [
        ['01', 'We show up prepared', 'The right tools, parts, and context to make the first visit count.'],
        ['02', 'We explain the why', 'Plain language. Honest options. No pressure to decide on the spot.'],
        ['03', 'We leave it better', 'Careful shoe covers, clean work areas, and respect for your home.'],
        ['04', 'We think long-term', 'Solutions designed for comfort today and fewer surprises tomorrow.'],
      ],
    },
    area: {
      eyebrow: 'Close to home',
      title1: 'Neighbors',
      title2: 'helping neighbors.',
    ...
    contact: {
      ...
      successCopy: 'Thanks for reaching out. A ComfortAir team member will review your note and follow up during business hours.',
      sentAgain: 'Send another request',
      formTitle: 'Request service',
      formCopy: 'A few details helps us make the first conversation useful.',
      nameLabel: 'Name',
      phoneLabel: 'Phone',
      helpLabel: 'What can we help with?',
      consentCopy: 'I agree to be contacted about this request. No marketing lists, no pressure.',
      submit: 'Send request',
    },
    footer: {
      description: 'A local HVAC team for the homes, neighborhoods, and weather we know best.',
      explore: 'Explore',
      exploreLinks: ['Services', 'Our approach', 'Customer reviews'],
      serviceArea: 'Service area',
      serviceAreaItems: ['Atlanta & Midtown', 'Decatur & East Cobb', 'Marietta & Roswell', 'North Metro Atlanta'],
      talk: 'Talk to us',
      phone: '(470) 555-0124',
      hours: 'Mon–Sat · 7:00am–7:00pm',
    ...
    chat: {
      ...
      greeting: 'Hi there — I’m ComfortAir’s virtual assistant. I can collect a few details for our team. I won’t estimate pricing or promise a technician time, but I can make your next step easier.',
      promptProblem: 'What is happening with your heating or air conditioning?',
      promptLocation: 'What city or ZIP code is the home in?',
      promptTiming: 'When would you ideally like help? (For example: today, this week, or flexible.)',
  ...
  es: {
    ...
    why: {
      ...
      features: [
        ['01', 'Llegamos preparados', 'Las herramientas, las piezas y el contexto correcto para que la primera visita cuente.'],
        ['02', 'Explicamos el porqué', 'Lenguaje claro. Opciones honestas. Sin presión para decidir en el momento.'],
        ['03', 'Lo dejamos mejor', 'Cuidamos los zapatos, mantenemos el área limpia y respetamos tu hogar.'],
        ['04', 'Pensamos a largo plazo', 'Soluciones diseñadas para el confort de hoy y menos sorpresas mañana.'],
      ],
    },
    area: {
    ...
    contact: {
      ...
      successCopy: 'Gracias por contactarnos. Un miembro del equipo ComfortAir revisará tu nota y hará seguimiento en horario comercial.',
      sentAgain: 'Enviar otra solicitud',
      formTitle: 'Solicitar servicio',
      formCopy: 'Unos pocos detalles ayudan a que la primera conversación sea útil.',
      nameLabel: 'Nombre',
      phoneLabel: 'Teléfono',
      helpLabel: '¿En qué podemos ayudarte?',
      consentCopy: 'Acepto ser contactado sobre esta solicitud. Sin listas de marketing, sin presión.',
      submit: 'Enviar solicitud',
    },
    footer: {
      description: 'Un equipo local de HVAC para los hogares, vecindarios y clima que conocemos mejor.',
      explore: 'Explorar',
      exploreLinks: ['Servicios', 'Nuestro enfoque', 'Reseñas de clientes'],
      serviceArea: 'Área de servicio',
      serviceAreaItems: ['Atlanta y Midtown', 'Decatur y East Cobb', 'Marietta y Roswell', 'Norte del metro de Atlanta'],
      talk: 'Contáctanos',
      phone: '(470) 555-0124',
      hours: 'Lun–Sáb · 7:00am–7:00pm',
    ...
    chat: {
      ...
      greeting: 'Hola — soy el asistente virtual de ComfortAir. Puedo recopilar algunos detalles para nuestro equipo. No estimaré precios ni prometo una hora de técnico, pero puedo hacer tu siguiente paso más sencillo.',
      promptProblem: '¿Qué está pasando con tu calefacción o aire acondicionado?',
      promptLocation: '¿En qué ciudad o código postal está la casa?',
      promptTiming: '¿Cuándo te gustaría idealmente recibir ayuda? (Por ejemplo: hoy, esta semana o flexible.)',
      promptDetails: 'Último paso: ¿cuál es tu nombre y el mejor número de teléfono?',
...
function Header({ onChat, lang, setLang }: { onChat: () => void; lang: Language; setLang: (lang: Language) => void }) {
  const [open, setOpen] = useState(false);
  const links = Object.entries(translations[lang].nav) as Array<[keyof typeof translations['en']['nav'], string]>;
  return <header className="absolute inset-x-0 top-0 z-30">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-10">
      <Logo light />
      <nav className="hidden items-center gap-6 md:flex">
        {links.map(([key, href]) => <a key={key} href={href} data-testid={`link-nav-${key}`} className="text-sm font-semibold text-[hsl(var(--background)/.75)] transition-colors hover:text-[hsl(var(--accent))]">{translations[lang].nav[key]}</a>)}
      ...
      <div className="hidden items-center gap-2 md:flex">
        {(['en', 'es'] as const).map(code => <button key={code} type="button" onClick={() => setLang(code)} data-testid={`button-lang-${code}`} className={`rounded-full px-3 py-2 text-sm font-semibold transition ${lang === code ? 'bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]' : 'text-[hsl(var(--background)/.85)] hover:text-[hsl(var(--accent))]'}`}>{code.toUpperCase()}</button>)}
      </div>
    ...
    {open && <div className="mx-4 rounded-2xl border border-[hsl(var(--background)/.15)] bg-[hsl(var(--primary)/.97)] p-4 shadow-xl md:hidden">
      {links.map(([key, href]) => <a key={key} href={href} onClick={() => setOpen(false)} data-testid={`link-mobile-${key}`} className="block border-b border-[hsl(var(--background)/.1)] px-3 py-3 text-sm font-bold text-[hsl(var(--background))]">{translations[lang].nav[key]}</a>)}
      <div className="mt-4 flex gap-2">
        {(['en', 'es'] as const).map(code => <button key={code} type="button" onClick={() => setLang(code)} data-testid={`button-mobile-lang-${code}`} className={`flex-1 rounded-full px-3 py-2 text-sm font-semibold transition ${lang === code ? 'bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]' : 'text-[hsl(var(--background)/.85)] hover:text-[hsl(var(--accent))]'}`}>{code.toUpperCase()}</button>)}
      </div>
      <button onClick={() => { setOpen(false); onChat(); }} data-testid="button-mobile-chat" className="mt-3 flex w-full items-center justify-between rounded-xl bg-[hsl(var(--accent))] px-4 py-3 text-sm font-bold text-[hsl(var(--accent-foreground))]">{translations[lang].headerChat} <MessageCircle size={17} /></button>
    </div>}
  </header>;
}

function Hero({ onChat, onRequest, lang }: { onChat: () => void; onRequest: () => void; lang: Language }) {
  ...
  return <section id="top" className="relative min-h-[760px] overflow-hidden bg-[hsl(var(--primary))] pt-28 text-[hsl(var(--background))]">
    ...
    <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 pb-20 lg:grid-cols-[1fr_410px] lg:px-10 lg:pb-28">
      <div className="max-w-3xl pt-16 lg:pt-24">
        <div className="reveal mb-7 flex items-center gap-3 font-mono-ui text-[10px] font-bold uppercase tracking-[.22em] text-[hsl(var(--accent))]"><span className="h-px w-9 bg-[hsl(var(--accent))]" /> {hero.eyebrow}</div>
        <h1 className="reveal reveal-delay-1 max-w-3xl font-display text-[clamp(4rem,8vw,7.2rem)] leading-[.84] tracking-[-.045em]">{hero.title1}<br /><em className="text-[hsl(var(--accent))]">{hero.title2}</em><br /><span className="text-[hsl(var(--background)/.83)]">{hero.title3}</span></h1>
        <p className="reveal reveal-delay-2 mt-8 max-w-md text-lg leading-relaxed text-[hsl(var(--background)/.75)]">{hero.subhead}</p>
        <div className="reveal reveal-delay-3 mt-9 flex flex-wrap gap-3"><Button onClick={onRequest} testId="button-hero-request">{hero.request}</Button><Button onClick={onChat} variant="outline" testId="button-hero-chat">{hero.chat}</Button></div>
        <div className="reveal reveal-delay-3 mt-10 flex flex-wrap gap-x-7 gap-y-3 text-xs font-semibold text-[hsl(var(--background)/.68)]">{hero.trust.map((item) => <span key={item} className="flex items-center gap-2"><CircleCheck size={15} className="text-[hsl(var(--accent))]" />{item}</span>)}</div>
      </div>
      <div className="reveal reveal-delay-2 relative hidden lg:block">
        <div className="float-slow absolute -left-20 top-7 z-10 w-48 rounded-2xl border border-[hsl(var(--background)/.17)] bg-[hsl(var(--background)/.1)] p-4 backdrop-blur-md"><div className="mb-3 flex items-center gap-2 text-[hsl(var(--accent))]"><ShieldCheck size={18} /><span className="font-mono-ui text-[9px] uppercase tracking-wider">{hero.peaceLabel}</span></div><p className="text-sm font-semibold leading-snug">{hero.peaceText}</p></div>
        <div className="relative ml-auto w-[350px] rounded-[2.4rem] bg-[hsl(var(--accent))] p-2 shadow-2xl shadow-[hsl(207_38%_8%/.3)]"><img src="https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=800&q=85" alt={hero.imageAlt} className="h-[490px] w-full rounded-[2rem] object-cover object-center mix-blend-multiply opacity-75" /><div className="absolute inset-2 rounded-[2rem] bg-gradient-to-t from-[hsl(var(--primary)/.8)] via-transparent to-transparent" /><div className="absolute bottom-8 left-8 right-8"><div className="mb-2 flex gap-1 text-[hsl(var(--accent))]">{[1, 2, 3, 4, 5].map(i => <Star key={i} size={14} fill="currentColor" />)}</div><p className="font-display text-2xl leading-none" dangerouslySetInnerHTML={{ __html: hero.quote }} /></div></div>
      </div>
    </div>
...
function WhyChooseUs({ lang }: { lang: Language }) {
  ...
  return <section id="why" className="overflow-hidden bg-[hsl(var(--secondary))] px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
    ...
    <div><p className="font-mono-ui text-[10px] font-bold uppercase tracking-[.22em] text-[hsl(var(--accent-foreground))]">{why.eyebrow}</p><h2 className="mt-4 max-w-xl font-display text-5xl leading-[.93] tracking-tight text-[hsl(var(--foreground))] lg:text-7xl">{why.title1}<br /><em className="text-[hsl(var(--primary))]">{why.title2}</em></h2><p className="mt-7 max-w-lg text-base leading-relaxed text-[hsl(var(--muted-foreground))]">{why.copy}</p><div className="mt-9 grid gap-5 sm:grid-cols-2">{why.features.map(([n, title, copy]) => <div key={n} className="border-t border-[hsl(var(--border))] pt-4"><div className="flex gap-4"><span className="font-mono-ui text-[10px] text-[hsl(var(--accent-foreground))]">{n}</span><div><h3 className="font-bold text-[hsl(var(--foreground))]">{title}</h3><p className="mt-2 text-sm leading-relaxed text-[hsl(var(--muted-foreground))]">{copy}</p></div></div></div>)}</div></div>
  </div></section>;
}
...
function Area({ lang }: { lang: Language }) {
  ...
  return <section id="area" className="bg-[hsl(var(--primary))] px-5 py-24 text-[hsl(var(--background))] lg:px-10 lg:py-28">
    <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.85fr_1.15fr]">
      <div>
        <p className="font-mono-ui text-[10px] font-bold uppercase tracking-[.22em] text-[hsl(var(--accent))]">{area.eyebrow}</p>
        <h2 className="mt-4 font-display text-5xl leading-[.9] lg:text-7xl">{area.title1}<br />helping <em>{area.title2}</em></h2>
        <p className="mt-7 max-w-md leading-relaxed text-[hsl(var(--background)/.68)]">{area.copy}</p>
        <a href="tel:4705550124" data-testid="link-area-phone" className="mt-8 inline-flex items-center gap-2 font-bold text-[hsl(var(--accent))] hover:underline"><Phone size={17} /> {area.phone}</a>
      </div>
...
function Reviews({ lang }: { lang: Language }) {
  ...
  return <section id="reviews" className="bg-[hsl(var(--background))] px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="font-mono-ui text-[10px] font-bold uppercase tracking-[.22em] text-[hsl(var(--accent-foreground))]">{reviewsText.eyebrow}</p><h2 className="mt-4 font-display text-5xl leading-[.9] text-[hsl(var(--foreground))] lg:text-7xl">{reviewsText.title1}<br /><em className="text-[hsl(var(--primary))]">{reviewsText.title2}</em></h2></div><div className="flex gap-2">{testimonials.map((item, i) => <button key={item.name} onClick={() => setActive(i)} data-testid={`button-review-${i}`} className={`size-11 rounded-full border text-xs font-bold transition-colors ${active === i ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary))] text-[hsl(var(--background))]' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--accent))]'}`}>0{i + 1}</button>)}</div></div><div className="mt-14 grid gap-8 border-t border-[hsl(var(--border))] pt-10 lg:grid-cols-[1fr_1.3fr] lg:items-center"><div className="flex items-center gap-4"><div className="grid size-14 place-items-center rounded-full bg-[hsl(var(--accent))] font-display text-xl text-[hsl(var(--accent-foreground))]">{review.initials}</div><div><p className="font-bold text-[hsl(var(--foreground))]">{review.name}</p><p className="text-sm text-[hsl(var(--muted-foreground))]">{review.location}</p></div><div className="ml-4 flex gap-1 text-[hsl(var(--accent))]">{[1, 2, 3, 4, 5].map(i => <Star key={i} size={13} fill="currentColor" />)}</div></div><blockquote className="font-display text-4xl leading-[.98] tracking-tight text-[hsl(var(--foreground))] lg:text-6xl">“{review.quote}”</blockquote></div></div></section>;
}

function Contact({ onChat, lang }: { onChat: () => void; lang: Language }) {
  const [sent, setSent] = useState(false);
  const contact = translations[lang].contact;
  return <section id="contact" className="bg-[hsl(var(--accent))] px-5 py-24 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.9fr_1.1fr]"><div><p className="font-mono-ui text-[10px] font-bold uppercase tracking-[.22em] text-[hsl(var(--accent-foreground)/.7)]">{contact.eyebrow}</p><h2 className="mt-4 font-display text-6xl leading-[.87] tracking-tight text-[hsl(var(--accent-foreground))] lg:text-8xl">{contact.title1}<br />{contact.title2}</h2><p className="mt-7 max-w-sm text-base leading-relaxed text-[hsl(var(--accent-foreground)/.72)]">{contact.copy}</p><div className="mt-9 flex flex-col gap-4"><a href="tel:4705550124" data-testid="link-contact-phone" className="flex items-center gap-3 text-xl font-bold text-[hsl(var(--accent-foreground))]"><Phone size={21} /> {contact.phone}</a><span className="flex items-center gap-3 text-sm font-semibold text-[hsl(var(--accent-foreground)/.7)]"><Clock3 size={18} /> {contact.hours}</span></div><button onClick={onChat} data-testid="button-contact-chat" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-[hsl(var(--accent-foreground))] underline decoration-[hsl(var(--accent-foreground)/.35)] underline-offset-4 hover:decoration-[hsl(var(--accent-foreground))]">{contact.chat} <MessageCircle size={16} /></button></div><div className="rounded-[2rem] bg-[hsl(var(--background))] p-6 shadow-xl lg:p-9">{sent ? <div className="flex min-h-[390px] flex-col items-center justify-center text-center"><div className="grid size-16 place-items-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><Check size={28} /></div><h3 className="mt-6 font-display text-4xl text-[hsl(var(--foreground))]">{contact.successTitle}</h3><p className="mt-3 max-w-sm text-sm leading-relaxed text-[hsl(var(--muted-foreground))]">{contact.successCopy}</p><button onClick={() => setSent(false)} data-testid="button-contact-reset" className="mt-7 text-sm font-bold text-[hsl(var(--primary))] underline underline-offset-4">{contact.sentAgain}</button></div> : <form onSubmit={(e) => { e.preventDefault(); setSent(true); }}><h3 className="font-display text-4xl text-[hsl(var(--foreground))]">{contact.formTitle}</h3><p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{contact.formCopy}</p><div className="mt-7 grid gap-5 sm:grid-cols-2"><label className="text-xs font-bold text-[hsl(var(--foreground))]">{contact.nameLabel}<input required data-testid="input-contact-name" className="mt-2 w-full rounded-xl border border-[hsl(var(--border))] bg-transparent px-4 py-3 text-sm outline-none transition-colors focus:border-[hsl(var(--primary))]" placeholder={lang === 'es' ? 'Tu nombre' : 'Your name'} /></label><label className="text-xs font-bold text-[hsl(var(--foreground))]">{contact.phoneLabel}<input required type="tel" data-testid="input-contact-phone" className="mt-2 w-full rounded-xl border border-[hsl(var(--border))] bg-transparent px-4 py-3 text-sm outline-none transition-colors focus:border-[hsl(var(--primary))]" placeholder="(470) 555-0124" /></label></div><label className="mt-5 block text-xs font-bold text-[hsl(var(--foreground))]">{contact.helpLabel}<textarea required data-testid="input-contact-message" className="mt-2 min-h-28 w-full resize-y rounded-xl border border-[hsl(var(--border))] bg-transparent px-4 py-3 text-sm outline-none transition-colors focus:border-[hsl(var(--primary))]" placeholder={lang === 'es' ? 'Cuéntanos qué hace (o no hace) tu sistema...' : 'Tell us what your system is doing (or not doing)...'} /></label><div className="mt-5 flex items-start gap-3 text-xs text-[hsl(var(--muted-foreground))]"><input type="checkbox" required data-testid="input-contact-consent" className="mt-0.5 accent-[hsl(var(--primary))]" /><span>{contact.consentCopy}</span></div><button type="submit" data-testid="button-contact-submit" className="mt-7 flex w-full items-center justify-center gap-2 rounded-full bg-[hsl(var(--primary))] px-5 py-3.5 text-sm font-bold text-[hsl(var(--background))] transition-transform hover:-translate-y-0.5">{contact.submit} <ArrowRight size={16} /></button></form>}</div></div></section>;
}
...
function ChatWidget({ open, onClose, lang }: { open: boolean; onClose: () => void; lang: Language }) {
  ...
  const handleUserResponse = (
    ...
    if (current === 'details') {
      ...
      const summary = chat.finalSummary
        .replace('{name}', name)
        .replace('{issue}', lead.problem || (lang === 'es' ? 'servicio HVAC' : 'HVAC service'))
        .replace('{location}', lead.location || (lang === 'es' ? 'tu área' : 'your area'))
  ...
  const quick = step === 'problem' ? chat.quickProblem : step === 'timing' ? chat.quickTiming : [];
  return <div className="fixed bottom-5 right-5 z-40 w-[min(calc(100vw-2rem),390px)] overflow-hidden rounded-3xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] shadow-2xl shadow-[hsl(207_38%_16%/.22)]" data-testid="widget-chat"><div className="flex items-center justify-between bg-[hsl(var(--primary))] px-5 py-4 text-[hsl(var(--background))]"><div className="flex items-center gap-3"><div className="relative grid size-10 place-items-center rounded-full bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]"><Sparkles size={18} /><span className="absolute -right-0.5 -top-0.5 size-2.5 rounded-full border-2 border-[hsl(var(--primary))] bg-emerald-400" /></div><div><p className="font-bold">{chat.assistantTitle}</p><p className="font-mono-ui text-[9px] uppercase tracking-wider text-[hsl(var(--background)/.55)]">{chat.assistantSubtitle}</p></div></div><button onClick={onClose} data-testid="button-chat-close" className="rounded-full p-1.5 text-[hsl(var(--background)/.65)] hover:bg-[hsl(var(--background)/.12)] hover:text-[hsl(var(--background))]"><X size={18} /></button></div><div className="max-h-[340px] min-h-[250px] space-y-3 overflow-y-auto p-4">{messages.map((message, i) => <div key={`${message.text}-${i}`} data-testid={`chat-message-${i}`} className={`max-w-[86%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${message.from === 'user' ? 'ml-auto rounded-br-sm bg-[hsl(var(--primary))] text-[hsl(var(--background))]' : 'rounded-bl-sm bg-[hsl(var(--secondary))] text-[hsl(var(--foreground))]'}`}>{message.text}</div>)}{step !== 'done' && quick.length > 0 && <div className="flex flex-wrap gap-2 pt-1">{quick.map(item => <button type="button" key={item} onClick={(event) => handleUserResponse(item, event)} disabled={processing} data-testid={`button-chat-quick-${item.toLowerCase().replaceAll(' ', '-')}`} className="rounded-full border border-[hsl(var(--primary)/.35)] px-3 py-1.5 text-xs font-bold text-[hsl(var(--primary))] transition-colors hover:bg-[hsl(var(--primary))] hover:text-[hsl(var(--background))]">{item}</button>)}</div>}</div>{step !== 'done' ? <form onSubmit={(event) => handleUserResponse(draft, event)} className="flex gap-2 border-t border-[hsl(var(--border))] p-3"><input id="chat-input" value={draft} onChange={e => setDraft(e.target.value)} disabled={processing} data-testid="input-chat-message" className="min-w-0 flex-1 rounded-xl border border-[hsl(var(--border))] bg-transparent px-3 py-2.5 text-sm outline-none focus:border-[hsl(var(--primary))]" placeholder={step === 'details' ? chat.placeholderDetails : chat.placeholderAnswer} /><button type="submit" disabled={processing} data-testid="button-chat-send" className="grid size-10 shrink-0 place-items-center rounded-xl bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))] hover:bg-[hsl(20_87%_51%)]"><Send size={16} /></button></form> : <div className="border-t border-[hsl(var(--border))] px-4 py-3 text-center text-xs text-[hsl(var(--muted-foreground))]">{chat.doneNote}</div>}</div>;
}

function Footer({ lang }: { lang: Language }) {
  const footer = translations[lang].footer;
  return <footer className="bg-[hsl(var(--primary))] px-5 pb-8 pt-16 text-[hsl(var(--background))] lg:px-10"><div className="mx-auto max-w-7xl"><div className="grid gap-10 border-b border-[hsl(var(--background)/.14)] pb-14 md:grid-cols-[1.3fr_1fr_1fr_1fr]"><div><Logo light /><p className="mt-6 max-w-xs text-sm leading-relaxed text-[hsl(var(--background)/.6)]">{footer.description}</p><div className="mt-6 flex gap-3"><a href="https://www.instagram.com" data-testid="link-instagram" aria-label="Instagram" className="rounded-full border border-[hsl(var(--background)/.18)] p-2.5 hover:border-[hsl(var(--accent))]"><Instagram size={15} /></a><a href="https://www.facebook.com" data-testid="link-facebook" aria-label="Facebook" className="rounded-full border border-[hsl(var(--background)/.18)] p-2.5 hover:border-[hsl(var(--accent))]"><Facebook size={15} /></a></div></div><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[hsl(var(--accent))]">{footer.explore}</p><div className="mt-5 flex flex-col gap-3 text-sm text-[hsl(var(--background)/.67)]">{footer.exploreLinks.map((link, index) => <a key={link} href={index === 0 ? '#services' : index === 1 ? '#why' : '#reviews'} data-testid={`link-footer-${index}`} className="hover:text-[hsl(var(--accent))]">{link}</a>)}</div></div><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[hsl(var(--accent))]">{footer.serviceArea}</p><div className="mt-5 flex flex-col gap-3 text-sm text-[hsl(var(--background)/.67)]">{footer.serviceAreaItems.map((item) => <span key={item}>{item}</span>)}</div></div><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[hsl(var(--accent))]">{footer.talk}</p><a href="tel:4705550124" data-testid="link-footer-phone" className="mt-5 block text-lg font-bold hover:text-[hsl(var(--accent))]">{footer.phone}</a><p className="mt-2 text-sm text-[hsl(var(--background)/.6)]">{footer.hours}</p><p className="mt-5 flex items-center gap-2 text-xs text-[hsl(var(--background)/.6)]"><ShieldCheck size={15} className="text-[hsl(var(--accent))]" /> {footer.licensed}</p></div></div><div className="flex flex-wrap justify-between gap-4 pt-7 font-mono-ui text-[9px] uppercase tracking-wider text-[hsl(var(--background)/.4)]"><span>{footer.copyright}</span><span>{footer.tagline}</span></div></div></footer>;
}
