...
function ChatWidget({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = (value: string) => {
    ...
    if (current === 'details') { const parts = value.split(','); const name = parts[0]?.trim() || value; const phone = parts.slice(1).join(',').trim() || 'Provided in conversation'; setLead(l => ({ ...l, name, phone })); setStep('done'); setMessages(m => [...m, { from: 'user', text: value.trim() }, { from: 'bot', text: `Thanks, ${name}. Here’s what I’ll pass to the ComfortAir team: ${lead.problem || 'HVAC service'} in ${lead.location || 'your area'}, ideally ${lead.timing || 'soon'}. They can follow up at ${phone}. A team member will review this during business hours — this chat does not confirm pricing or an appointment time.` }]); }
  };
